...
docker run --rm -i -v "${VOLUME_DIR}":/build -w /build pcmagas/arch-pkg-builder build_n_run
docker run --rm -i -v "${VOLUME_DIR}":/build -w /build pcmagas/arch-pkg-builder makepkg --printsrcinfo > ${VOLUME_DIR}/.SRCINFO
